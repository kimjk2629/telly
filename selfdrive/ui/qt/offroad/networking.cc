...
void Networking::wrongPassword(const QString &ssid) {
  if (wifi->seenNetworks.contains(ssid)) {
    ...
    QString pass = InputDialog::getText("암호가 틀렸습니다", this, "for \"" + n.ssid +"\"", true, 8);
    if (!pass.isEmpty()) {
      wifi->connect(n, pass);
    }
  }
...
AdvancedNetworking::AdvancedNetworking(QWidget* parent, WifiManager* wifi): QWidget(parent), wifi(wifi) {
  ...
  tetheringToggle = new ToggleControl("테더링 켜기", "", "", wifi->isTetheringEnabled());
  list->addItem(tetheringToggle);
  QObject::connect(tetheringToggle, &ToggleControl::toggleFlipped, this, &AdvancedNetworking::toggleTethering);

  // Change tethering password
  ButtonControl *editPasswordButton = new ButtonControl("테더링 암호", "편집");
  connect(editPasswordButton, &ButtonControl::clicked, [=]() {
    QString pass = InputDialog::getText("새로운 암호를 입력하세요", this, "", true, 8, wifi->getTetheringPassword());
    if (!pass.isEmpty()) {
  ...
  ToggleControl *roamingToggle = new ToggleControl("로밍 켜기", "", "", roamingEnabled);
  ...
  // APN settings
  ButtonControl *editApnButton = new ButtonControl("APN 설정", "편집");
  connect(editApnButton, &ButtonControl::clicked, [=]() {
    const bool roamingEnabled = params.getBool("GsmRoaming");
    const QString cur_apn = QString::fromStdString(params.get("GsmApn"));
    QString apn = InputDialog::getText("Enter APN", this, "자동설정을 하려면 공백으로", false, -1, cur_apn).trimmed();
...
WifiUI::WifiUI(QWidget *parent, WifiManager* wifi) : QWidget(parent), wifi(wifi) {
  ...
  QLabel *scanning = new QLabel("네트워크를 찾는 중");
...
void WifiUI::refresh() {
  ...
  for (Network &network : sortedNetworks) {
    ...
    if (network.connected == ConnectedType::CONNECTING) {
      QPushButton *connecting = new QPushButton("접속중...");
    ...
    if (wifi->isKnownConnection(network.ssid) && !wifi->isTetheringEnabled()) {
      QPushButton *forgetBtn = new QPushButton("제거");
      forgetBtn->setObjectName("forgetBtn");
      QObject::connect(forgetBtn, &QPushButton::clicked, [=]() {
        if (ConfirmationDialog::confirm("다음 네트워크를 제거할까요 \"" + QString::fromUtf8(network.ssid) + "\"?", this)) {
